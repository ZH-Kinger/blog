...
![](https://cdn.nlark.com/yuque/0/2026/png/62301513/1773646133200-4fbc47f3-88ca-473e-bf91-78dc2c04c5fe.png)
<hr id="xIZ4t" class="ne-hr">
## 1. 为什么需要 RDMA？（传统网络的“痛”）

在传统的 **TCP/IP** 网络中，数据搬运是一个非常累人的过程：


- **多次拷贝**：数据要从网卡拷到内核内存，再从内核内存拷到用户程序的内存。
- **CPU 疯狂打工**：每一次数据包的拆解、封装和拷贝，都需要 CPU 亲自下场处理中断。
- **高延迟**：这一来二去，时间都浪费在“搬运”和“等待 CPU”上了。

在 AI 训练时，如果你有 1000 块 GPU 要交换数据，如果还用 TCP/IP，那 CPU 全在搬运数据了，根本没空管模型。
<hr id="lTA6F" class="ne-hr">
## 2. RDMA 是怎么解决的？（“顺丰直达”）
...
<hr id="i2GWG" class="ne-hr">
## 3. RDMA 的三大支柱

要实现这种“直达”效果，需要依靠：


- **RNIC (RDMA 网卡)**：特殊的网卡硬件，具备直接读写内存的逻辑。
- **队列对 (Queue Pair, QP)**：发送端和接收端建立的直接通信通道。
- **内存注册 (Memory Registration)**：程序必须提前告诉网卡，哪块内存区域是允许远程访问的，以保证安全。
<hr id="MNB3S" class="ne-hr">
## 4. RDMA 的三种实现方式

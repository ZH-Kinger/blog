...
- **自动提取**：它读取函数的名称 get_weather。
- **自动生成 Schema**：它把你的文档字符串（Docstring）提取出来，直接转成 **JSON Schema** 里的 description。
- **类型检查**：它根据 city: str 自动生成 Schema 里的 type: "string"。

**这就是为什么用了装饰器，你就不用手写那几百行枯燥的 JSON 了。**
<hr id="eVhJZ" class="ne-hr">
## 5. 总结：装饰器的三步走


- **定义**：写一个接收函数并返回新函数的“外壳”。
- **套用**：在目标函数上方写上 @装饰器名。
- **执行**：像平常一样调用原函数，它会自动带上“外壳”里的功能。
<hr id="mXfFd" class="ne-hr">
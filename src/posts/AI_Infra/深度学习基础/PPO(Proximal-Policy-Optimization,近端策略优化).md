...
- **任务**：针对 Actor 吐出来的完整回答，在最后打出一个绝对的分数（比如 ![](https://cdn.nlark.com/yuque/__latex/0ee8b263119952b72c1c6e54ef348483.svg) 或 ![](https://cdn.nlark.com/yuque/__latex/1a98dcdb2efed799f4620934760f223a.svg)）。
...
PPO 会计算新策略（当前的 Actor）和老策略（上一轮的 Actor）输出某个 Token 的概率比值 ![](https://cdn.nlark.com/yuque/__latex/4ba2313b02bf2984b153fa0441a58a38.svg)。
...
PPO 的 Loss 函数强行引入了一个超参数 ![](https://cdn.nlark.com/yuque/__latex/7c102e7a7d231bf935f9bc23417779a8.svg)（通常设为 `0.1` 或 `0.2`）。

- 如果算法发现新策略的膨胀程度超出了 ![](https://cdn.nlark.com/yuque/__latex/8d2c8b17d61700080073de56f0bbe6e5.svg)（比如超过了 1.2 倍），公式内部的 `clip` 函数就会**强行把多出来的部分削平，不给任何额外的梯度回传**。
